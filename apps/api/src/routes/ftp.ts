...
import { hashPassword, verifyPassword } from '../lib/ftp/password';
...
// =============================================================================
// Route Handler: POST /api/ftp/auth
// =============================================================================

async function handleFtpAuth(c: any) {
  return safeTry(async function* () {
    ...
    if (!credential) {
      return err<never, HandlerError>({ code: 'UNAUTHENTICATED', message: 'Invalid credentials' });
    }

    // 2. Verify password hash
    const passwordValid = yield* ResultAsync.fromThrowable(
      () => verifyPassword(credential.passwordHash, password),
      (e): HandlerError => ({ code: 'INTERNAL_ERROR', message: 'Authentication error', cause: e }),
    )();
    ...
    if (new Date(credential.expiresAt) < new Date()) {
      return err<never, HandlerError>({ code: 'UNAUTHENTICATED', message: 'Credentials expired' });
    }
...
// =============================================================================
// Route Handler: POST /api/ftp/presign
// =============================================================================

async function handleFtpPresign(c: any) {
  return safeTry(async function* () {
    const db = c.var.db();
    const ftpAuth = (c as any).get?.('ftpAuth') as any;

    if (!ftpAuth) {
      return err<never, HandlerError>({ code: 'UNAUTHENTICATED', message: 'Not authenticated' });
    }
...
// =============================================================================
// Route Handler: POST /events/:id/ftp-credentials (Photographer auth)
// =============================================================================

async function handleCreateFtpCredentials(c: any) {
  return safeTry(async function* () {
...
// =============================================================================
// Route Handler: GET /events/:id/ftp-credentials (Photographer auth)
// =============================================================================

async function handleGetFtpCredentials(c: any) {
  return safeTry(async function* () {
    const photographer = c.var.photographer;
    const db = c.var.db();
    const { id: eventId } = c.req.valid('param');
    ...
    const credential = credentialRows[0];
    if (!credential) {
      return err<never, HandlerError>({ code: 'NOT_FOUND', message: 'FTP credentials not found' });
    }
...
// =============================================================================
// Route Handler: DELETE /events/:id/ftp-credentials (Photographer auth)
// =============================================================================

async function handleDeleteFtpCredentials(c: any) {
  return safeTry(async function* () {
    const photographer = c.var.photographer;
    const db = c.var.db();
    const { id: eventId } = c.req.valid('param');
...
export const ftpRouter = new Hono<Env>()
  .post('/auth', zValidator('json', ftpAuthRequestSchema), handleFtpAuth)
  .post(
    '/presign',
    async (c, next) => {
  ...
  .get(
    ...
    requirePhotographer(),
    zValidator('param', ftpCredentialsParamsSchema),
    handleGetFtpCredentials,
  )
  .delete(
    '/events/:id/ftp-credentials',
    requirePhotographer(),
    zValidator('param', ftpCredentialsParamsSchema),
    handleDeleteFtpCredentials,
  );
