...
function getTerraformWebhookUrl() {
  try {
    ...
    const match = content.match(/^webhook_url\s*=\s*\"(.+)\"\s*$/m);
...
const terraformWebhookUrl = getTerraformWebhookUrl();
...
if (terraformWebhookUrl && !terraformWebhookUrl.includes(ngrokDomain)) {
  console.warn(
    '⚠️  Your Terraform dev notification proxy WEBHOOK_URL does not match NGROK_DOMAIN.',
  );
  console.warn(`   terraform.tfvars webhook_url: ${terraformWebhookUrl}`);
  console.warn(`   .dev.vars NGROK_DOMAIN:       ${ngrokDomain}`);
  console.warn('   R2 notifications may forward to the wrong machine.\n');
}
...
const ngrok = spawn(
  'ngrok',
  ['http', '--domain', ngrokDomain, '--log=stdout', '--log-format=logfmt', '8081'],
  {
    stdio: 'inherit',
    shell: true,
  },
