/**
 * Test Image Generator
 *
 * Generates valid image data for testing.
 * TODO: Later, download real test images from R2 for face detection tests.
 */

/**
 * Generate a minimal valid JPEG image.
 * This creates a 1x1 pixel red JPEG - valid for API testing but contains no faces.
 *
 * For actual face detection tests, use real images from R2.
 */
export function generateTestImage(): ArrayBuffer {
  // Minimal valid JPEG: 1x1 pixel, red color
  // This is a hand-crafted minimal JPEG that AWS Rekognition will accept
  const jpegBytes = new Uint8Array([
    // SOI (Start of Image)
    0xff, 0xd8,
    // APP0 (JFIF marker)
    0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00,
    0x00, 0x01, 0x00, 0x01, 0x00, 0x00,
    // DQT (Define Quantization Table)
    0xff, 0xdb, 0x00, 0x43, 0x00, 0x08, 0x06, 0x06, 0x07, 0x06, 0x05, 0x08,
    0x07, 0x07, 0x07, 0x09, 0x09, 0x08, 0x0a, 0x0c, 0x14, 0x0d, 0x0c, 0x0b,
    0x0b, 0x0c, 0x19, 0x12, 0x13, 0x0f, 0x14, 0x1d, 0x1a, 0x1f, 0x1e, 0x1d,
    0x1a, 0x1c, 0x1c, 0x20, 0x24, 0x2e, 0x27, 0x20, 0x22, 0x2c, 0x23, 0x1c,
    0x1c, 0x28, 0x37, 0x29, 0x2c, 0x30, 0x31, 0x34, 0x34, 0x34, 0x1f, 0x27,
    0x39, 0x3d, 0x38, 0x32, 0x3c, 0x2e, 0x33, 0x34, 0x32,
    // SOF0 (Start of Frame - Baseline DCT)
    0xff, 0xc0, 0x00, 0x0b, 0x08, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x11,
    0x00,
    // DHT (Define Huffman Table - DC)
    0xff, 0xc4, 0x00, 0x1f, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01,
    0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02,
    0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
    // DHT (Define Huffman Table - AC)
    ,
    // SOS (Start of Scan)
    0xff, 0xda, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3f, 0x00, 0x7f, 0xff,
    0xd9, // EOI (End of Image)
  ]);

  return jpegBytes.buffer;
}

/**
 * Generate a larger test image (100x100 pixels).
 * Still no face, but more realistic size for testing.
 */
export function generateLargerTestImage(): ArrayBuffer {
  // For now, return the minimal image
  // TODO: Generate actual 100x100 image or download from R2
  return generateTestImage();
}
