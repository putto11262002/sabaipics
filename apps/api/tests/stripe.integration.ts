...
describe("Stripe Integration", () => {
  ...
  describe("Customer API", () => {
    it(
      "creates a new customer with metadata",
      async () => {
    ...
    it(
      "finds customer by photographer ID",
      async () => {
        // Give Stripe search index time to update
        await new Promise((resolve) => setTimeout(resolve, 2000));

        const customer = await findCustomerByPhotographerId(
          stripe,
          testPhotographerId
        );
  ...
  describe("Checkout Session API", () => {
    ...
    it(
      "creates session with multiple line items",
      async () => {
        expect(createdCustomerId).not.toBeNull();
    ...
    it(
      "handles THB currency correctly",
      async () => {
        expect(createdCustomerId).not.toBeNull();
