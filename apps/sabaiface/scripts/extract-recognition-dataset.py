...
def parse_ground_truth(dataset_path):
    ...
    image_to_people = defaultdict(
        lambda: defaultdict(set)
    )  # image_name → {full_path: set of person_ids}
...
def select_people(person_to_images, num_people, min_images):
    ...
    qualified = {
        pid: dict(images)
        for pid, images in person_to_images.items()
        if len(images) >= min_images
    }

    if len(qualified) < num_people:
        print(
            f"  ❌ Not enough people! Only {len(qualified)} have {min_images}+ images"
        )
        counts = sorted([len(imgs) for imgs in person_to_images.values()], reverse=True)
        print(
            f"  Max: {counts[0] if counts else 0}, Median: {counts[len(counts) // 2] if counts else 0}"
        )
        sys.exit(1)
...
def create_splits(selected_people, image_to_people, images_per_person, index_ratio):
    ...
    """
    print(
        f"✂️  Creating splits: {int(index_ratio * 100)}% index, {int((1 - index_ratio) * 100)}% query"
    )
    ...
    # First pass: select index images for all people
    person_index_names = {}
    for person_id, name_to_path in selected_people.items():
        image_names = list(name_to_path.keys())
        person_index_names[person_id] = random.sample(
            image_names, min(index_size, len(image_names))
        )

        # Add to global index set (deduplicated)
        for name in person_index_names[person_id]:
            all_index_images[name] = name_to_path[name]

    # Second pass: select query images ensuring NO overlap with global index set
    global_index_set = set(all_index_images.keys())
    for person_id, name_to_path in selected_people.items():
        image_names = list(name_to_path.keys())

        # Query images must be from images NOT in the global index set
        available_for_query = [
            img for img in image_names if img not in global_index_set
        ]
        person_query_names = random.sample(
            available_for_query, min(query_size, len(available_for_query))
        )
        ...
        identities[str(person_id)] = {
            ...
            "indexImageCount": len(person_index_names[person_id]),
            "queryImageCount": len(person_query_names),
            "indexImages": sorted(person_index_names[person_id]),
            "indexImagePaths": [
                name_to_path[name] for name in sorted(person_index_names[person_id])
            ],
    ...
    index_set = [
        {"name": name, "path": path} for name, path in sorted(all_index_images.items())
    ]
    ...
    # Verify no overlap between index and query sets
    index_images = set()
    for img_obj in index_set:
        index_images.add(img_obj["name"])

    query_images = set()
    for person_id, person_data in identities.items():
        for img_name in person_data["queryImages"]:
            query_images.add(img_name)

    overlap = index_images & query_images
    ...
    print(f"  🔍 Overlap check: {len(overlap)} images in both sets\n")
    ...
    if overlap:
        print(f"  ❌ ERROR: {len(overlap)} images appear in both index and query sets!")
        print(f"     Overlapping images: {list(overlap)}\n")
...
def save_dataset(dataset, dataset_path):
    ...
    print(
        f"   Total unique index:  {dataset['metadata']['totalUniqueIndexImages']} images"
    )
...
def main():
    ...
    if not dataset_path:
        ...
        print(
            "     python3 scripts/extract-recognition-dataset.py --dataset /path/to/kaggle"
        )
        print()
