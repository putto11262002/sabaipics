...
export const eventsRouter = new Hono<Env>().get(
  '/',
  requirePhotographer(),
  requireConsent(),
  async (c) => {
...
await db.update(events).set({ qrCodeR2Key: qrKey }).where(eq(events.id, eventId));
```
...
if (event.photographerId !== photographer.id) {
  return c.json(createAuthError('FORBIDDEN', 'You do not have access to this event'), 403);
}
```
