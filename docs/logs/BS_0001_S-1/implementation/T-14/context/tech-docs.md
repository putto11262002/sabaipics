...
export async function generateEventQR(options: GenerateEventQROptions): Promise<Uint8Array> {
...
export const eventsRouter = new Hono<Env>().post('/', requirePhotographer(), async (c) => {
  const env = c.env;
  ...
  await db.update(events).set({ qrCodeR2Key: qrKey }).where(eq(events.id, event.id));
