...
const result = await db
  ...
  .where(and(eq(photos.eventId, eventId), inArray(photos.id, photoIds), isNull(photos.deletedAt)))
...
await db.update(events).set({ rekognitionCollectionId: null }).where(eq(events.id, eventId));
```
...
for (const event of expiredEvents) {
  try {
    ...
    await db.update(events).set({ rekognitionCollectionId: null }).where(eq(events.id, event.id));
...
export async function createCollection(client: RekognitionClient, eventId: string): Promise<string>;
