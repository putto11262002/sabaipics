...
const recentUploads = await db
  ...
  .from(photos)
  .where(and(eq(photos.eventId, eventId), gt(photos.uploadedAt, sql`NOW() - INTERVAL '1 hour'`)));
...
```typescript
await db.transaction(async (tx) => {
  const [row] = await tx.select().from(photographers).where(eq(photographers.id, id)).for('update'); // Does this work?
});
```
