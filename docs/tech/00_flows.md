# System Flows

**Status:** Draft
**Last Updated:** 2025-12-03

---

## Overview

This document describes the step-by-step flows for key operations. Each flow shows the sequence of actions across system components.

**Components:**
- **Client**: Web app, Desktop app, Lightroom plugin
- **API**: Hono on Cloudflare Workers
- **DB**: Postgres on Neon
- **R2**: Cloudflare R2 object storage
- **Rekognition**: AWS face detection/search
- **LINE**: LINE Messaging API
- **Clerk**: Authentication
- **Stripe**: Payments

---

## 1. Photographer Signup Flow

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │  Clerk  │     │   API   │     │   DB    │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │
     │  1. Click "Sign in with LINE/Google/Email"    │
     │──────────────►│               │               │
     │               │               │               │
     │  2. OAuth flow / OTP          │               │
     │◄─────────────►│               │               │
     │               │               │               │
     │  3. Return JWT + user info    │               │
     │◄──────────────│               │               │
     │               │               │               │
     │  4. Call API with JWT         │               │
     │───────────────────────────────►               │
     │               │               │               │
     │               │  5. Verify JWT with Clerk     │
     │               │◄──────────────│               │
     │               │               │               │
     │               │  6. Check if photographer exists
     │               │               │──────────────►│
     │               │               │               │
     │               │               │  7. If new: create photographer record
     │               │               │  (status: pending_consent)
     │               │               │──────────────►│
     │               │               │               │
     │               │               │  8. If new: add signup bonus credits
     │               │               │──────────────►│
     │               │               │               │
     │  9. Return profile (status: pending_consent)  │
     │◄──────────────────────────────│               │
     │               │               │               │
     │  10. Frontend shows onboarding page           │
     │  "Before you start..."        │               │
     │  - Checkbox (Thai + English): │               │
     │    "I confirm I obtained consent from         │
     │     photographed individuals. [Privacy Policy]"
     │               │               │               │
     │  11. User checks box, clicks "Complete Setup" │
     │               │               │               │
     │  12. POST /api/photographers/me/complete-onboarding
     │───────────────────────────────►               │
     │               │               │               │
     │               │               │  13. Insert consent_record
     │               │               │  (user_type: photographer,
     │               │               │   consent_type: photographer_registration)
     │               │               │──────────────►│
     │               │               │               │
     │               │               │  14. Update photographer
     │               │               │  (status: active)
     │               │               │──────────────►│
     │               │               │               │
     │  15. Return success, redirect to dashboard    │
     │◄──────────────────────────────│               │
     │               │               │               │
```

**PDPA Compliance:**
- Photographer must consent before accessing dashboard
- Consent stored with timestamp, IP address, privacy policy version
- Privacy Policy explains photographer's obligation to obtain consent from event participants

**Notes:**
- Clerk handles all OAuth/OTP complexity
- We sync profile data (name, avatar) from Clerk on each login
- Signup bonus credits added on first login (if configured)
- Dashboard access blocked until onboarding complete (status: active)

---

## 2. Credit Purchase Flow

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │   API   │     │ Stripe  │     │   DB    │     │ Webhook │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │               │
     │  1. Select package (e.g., STARTER 500 credits)│               │
     │──────────────►│               │               │               │
     │               │               │               │               │
     │               │  2. Create Stripe Checkout Session             │
     │               │──────────────►│               │               │
     │               │               │               │               │
     │               │  3. Return checkout URL       │               │
     │               │◄──────────────│               │               │
     │               │               │               │               │
     │  4. Redirect to Stripe Checkout               │               │
     │◄──────────────│               │               │               │
     │               │               │               │               │
     │  5. User completes payment    │               │               │
     │──────────────────────────────►│               │               │
     │               │               │               │               │
     │               │               │  6. Webhook: payment_intent.succeeded
     │               │               │───────────────────────────────►│
     │               │               │               │               │
     │               │               │               │  7. Create payment record
     │               │               │               │◄──────────────│
     │               │               │               │               │
     │               │               │               │  8. Add credit ledger entry
     │               │               │               │  (type=purchase, expires_at=+6mo)
     │               │               │               │◄──────────────│
     │               │               │               │               │
     │  9. Redirect to success page  │               │               │
     │◄──────────────────────────────│               │               │
     │               │               │               │               │
```

**Notes:**
- Credits only added after Stripe webhook confirms payment
- Webhook handler is idempotent (checks for existing payment)
- `expires_at` set to 6 months from purchase

---

## 3. Create & Publish Event Flow

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │   API   │     │   DB    │     │  Rekog  │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │
     │  1. Create event (name, dates)│               │
     │──────────────►│               │               │
     │               │               │               │
     │               │  2. Validate dates            │
     │               │  (end > start, start not past)│
     │               │               │               │
     │               │  3. Insert event (status=draft)
     │               │──────────────►│               │
     │               │               │               │
     │  4. Return event              │               │
     │◄──────────────│               │               │
     │               │               │               │
     │  ... configure settings ...   │               │
     │               │               │               │
     │  5. Publish event             │               │
     │──────────────►│               │               │
     │               │               │               │
     │               │  6. Create Rekognition collection
     │               │───────────────────────────────►│
     │               │               │               │
     │               │  7. Update event              │
     │               │  (status=published, collection_id)
     │               │──────────────►│               │
     │               │               │               │
     │  8. Return published event    │               │
     │◄──────────────│               │               │
     │               │               │               │
```

**Notes:**
- Rekognition collection created on publish, not on create
- Collection naming: `facelink-{event_uuid}`
- Draft events have no collection (saves cost)

---

## 4. Photo Upload Flow (Web/Desktop)

```
┌─────────┐     ┌─────────┐     ┌────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │   API   │     │  D.O.  │     │   DB    │     │   R2    │     │  Rekog  │
└────┬────┘     └────┬────┘     └────┬───┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │               │               │
     │  1. Upload photo              │               │               │               │
     │──────────────►│               │               │               │               │
     │               │               │               │               │               │
     │               │  2. CHECK RATE LIMIT          │               │               │
     │               │  (per-user, Durable Object)   │               │               │
     │               │──────────────►│               │               │               │
     │               │               │               │               │               │
     │               │  If exceeded → 429            │               │               │
     │               │◄──────────────┘               │               │               │
     │               │               │               │               │               │
     │               │  3. VALIDATE                  │               │               │
     │               │  - event.status = published   │               │               │
     │               │  - now >= start_datetime      │               │               │
     │               │  - now <= end_datetime        │               │               │
     │               │  - credit_balance >= 1        │               │               │
     │               │  - valid image type/size      │               │               │
     │               │               │               │               │               │
     │               │  4. Deduct 1 credit (FIFO)    │               │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │               │  5. Generate R2 key           │               │               │
     │               │  6. Upload to R2              │               │               │
     │               │───────────────────────────────────────────────►               │
     │               │               │               │               │               │
     │               │  7. Insert photo (status=pending)             │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │               │  8. Write audit log (deliveries)              │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │  9. Return photo (pending)    │               │               │               │
     │◄──────────────│               │               │               │               │
     │               │               │               │               │               │
     │               │  10. ASYNC: Process photo (queue)             │               │
     │               │               │               │               │               │
     │               │  11. Update status=processing │               │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │               │  12. IndexFaces (detects + indexes faces)     │               │
     │               │───────────────────────────────────────────────────────────────►
     │               │               │               │               │               │
     │               │  13. Insert face records      │               │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │               │  14. Update photo             │               │               │
     │               │  (status=ready, face_count)   │               │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │               │  15. Update event stats       │               │               │
     │               │───────────────────────────────►               │               │
     │               │               │               │               │               │
     │  16. WebSocket: photo ready   │               │               │               │
     │      → See 06_websocket.md for pattern                                       │
     │◄──────────────│               │               │               │               │
     │               │               │               │               │               │
```

**Key validation order:**
1. **Rate limit check** (Durable Object) → `RATE_LIMITED` (429)
2. Event published? → `EVENT_NOT_PUBLISHED`
3. Upload window open? → `EVENT_NOT_STARTED` or `EVENT_UPLOAD_CLOSED`
4. Has credits? → `INSUFFICIENT_CREDITS`
5. Valid file? → `INVALID_FILE_TYPE` or `FILE_TOO_LARGE`

**Important sequencing:**
- **Rate limit checked FIRST** - fast check (<1ms), prevents wasted work
- **Credit deduction happens BEFORE R2 upload** - ensures atomicity
- **Audit log written AFTER upload** - permanent business record (Neon Postgres)

---

## 5. FTP Upload Flow

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Camera  │     │FTP (VPS)│     │   API   │     │ ...rest │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │
     │  1. FTP connect (credentials) │               │
     │──────────────►│               │               │
     │               │               │               │
     │               │  2. Validate credentials      │
     │               │  (photographer_id + event_id) │
     │               │──────────────►│               │
     │               │               │               │
     │  3. FTP login success         │               │
     │◄──────────────│               │               │
     │               │               │               │
     │  4. Upload file via FTP       │               │
     │──────────────►│               │               │
     │               │               │               │
     │               │  5. Forward to API            │
     │               │  (same as web upload)         │
     │               │──────────────►│               │
     │               │               │               │
     │               │               │  ... same as flow 4 ...
     │               │               │               │
```

**FTP credentials format:**
- Username: `{photographer_id}_{event_id}`
- Password: Generated token per event

**FTP server role:** Thin proxy that forwards to API. No local storage.

---

## 6. Face Search Flow (Web - Anonymous)

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │CF WAF   │     │   API   │     │   DB    │     │  Rekog  │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │               │
     │  1. Submit selfie for event   │               │               │
     │──────────────►│               │               │               │
     │               │               │               │               │
     │               │  2. CHECK RATE LIMIT          │               │
     │               │  (per-IP: 600/min, Cloudflare WAF)            │
     │               │  (global: 1800/min, Cloudflare WAF)           │
     │               │               │               │               │
     │               │  If exceeded → 429            │               │
     │               │               │               │               │
     │               │  3. Forward to API            │               │
     │               │──────────────►│               │               │
     │               │               │               │               │
     │               │               │  4. VALIDATE                  │
     │               │               │  - event exists & published   │
     │               │               │  - event not expired          │
     │               │               │──────────────►│               │
     │               │               │               │               │
     │               │               │  5. CHECK CONSENT             │
     │               │               │  If logged in: check existing consent
     │               │               │  If anonymous: always require consent
     │               │               │──────────────►│               │
     │               │               │               │               │
     │  6. If no consent: show dialog                │               │
     │  Checkbox (Thai + English):   │               │               │
     │  "I consent to facial recognition processing. │               │
     │   [Privacy Policy]"           │               │               │
     │               │               │               │               │
     │  7. User checks box, submits  │               │               │
     │───────────────────────────────►               │               │
     │               │               │               │               │
     │               │               │  8. Insert consent_record     │
     │               │               │  (user_id: NULL for anonymous,
     │               │               │   search_session_id: TBD)
     │               │               │──────────────►│               │
     │               │               │               │               │
     │               │               │  9. Check cache (event_id + face_hash)
     │               │               │  If cached: return cached results
     │               │               │               │               │
     │               │               │  10. Store selfie temporarily │
     │               │               │  (R2 temp prefix, 1 hour TTL) │
     │               │               │               │               │
     │               │               │  11. SearchFacesByImage       │
     │               │               │───────────────────────────────►
     │               │               │               │               │
     │               │               │  12. Get matching face_ids + similarity
     │               │               │◄──────────────────────────────│
     │               │               │               │               │
     │               │               │  13. Lookup photos by face_ids│
     │               │               │──────────────►│               │
     │               │               │               │               │
     │               │               │  14. Create search_session    │
     │               │               │  (participant_id = NULL for anon)
     │               │               │──────────────►│               │
     │               │               │               │               │
     │               │               │  15. Create search_results    │
     │               │               │──────────────►│               │
     │               │               │               │               │
     │               │               │  16. Cache results            │
     │               │               │               │               │
     │  17. Return matching photos   │               │               │
     │◄──────────────────────────────────────────────│               │
     │               │               │               │               │
```

**Key validation order:**
1. **Rate limit check** (Cloudflare WAF) → `RATE_LIMITED` (429)
   - 600 requests/min per IP
   - 1800 requests/min global
2. Event exists & published? → `EVENT_NOT_FOUND` or `EVENT_NOT_PUBLISHED`
3. Event not expired? → `EVENT_EXPIRED`
4. **Consent given?** → Show consent dialog if not

**PDPA Compliance:**
- Anonymous users: Always show consent (can't track between sessions)
- Logged-in users: Check if consent already given for this event, only ask once
- Consent stored with search_session_id (for anonymous) or participant_id (for logged-in)
- Privacy Policy explains: facial recognition, cross-border transfer (USA), 1-month retention

**Notes:**
- No login required for web search
- Rate limiting via Cloudflare WAF (per-IP + global)
- Cache key: hash of (event_id + selfie_face_embedding)
- Cache TTL: Event lifetime (1 month from start) - persists until event expires
- `participant_id` is NULL for anonymous searches
- Selfie stored temporarily in R2 with 1 hour auto-delete

---

## 7. Photo Download Flow (Web - Anonymous)

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│ Client  │     │   API   │     │   DB    │     │ R2/CDN  │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │
     │  1. Request download (photo_ids)              │
     │──────────────►│               │               │
     │               │               │               │
     │               │  2. Validate event not expired│
     │               │──────────────►│               │
     │               │               │               │
     │               │  3. Get photo R2 keys         │
     │               │──────────────►│               │
     │               │               │               │
     │               │  4. Generate signed URLs      │
     │               │  (or direct CDN URLs)         │
     │               │               │               │
     │  5. Return download URLs      │               │
     │◄──────────────│               │               │
     │               │               │               │
     │  6. Download from CDN         │               │
     │───────────────────────────────────────────────►
     │               │               │               │
     │  7. Photos    │               │               │
     │◄──────────────────────────────────────────────│
     │               │               │               │
```

**Notes:**
- No login required
- Photos served via Cloudflare CDN
- For multiple photos: return individual URLs or generate ZIP

---

## 8. LINE Notification Flow (Web - "Send to LINE" Button)

→ See `06_line_messaging.md` for integration pattern

**VERIFIED:** Based on competitor (Pixid) implementation - this is the standard Thai market flow.

**Two requirements for sending photos via LINE:**
1. User must sign in with LINE (we get `userId`)
2. User must be friend of our OA (otherwise message silently fails)

### 8a. New User Flow (Not signed in, not friend)

```
┌─────────────────────────────────────────────────────────────────────────┐
│  PARTICIPANT WEB APP (after face search, viewing results)              │
│                                                                         │
│  [Download Photos]     [ส่งรูปผ่าน Line / Send via LINE]               │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼ User clicks "Send via LINE"
┌─────────────────────────────────────────────────────────────────────────┐
│  STEP 2: LINE Login Screen (access.line.me)                            │
│                                                                         │
│  ┌─────────────────────────────────────────┐                           │
│  │  LINE                                   │                           │
│  │  ─────────────────────────────────────  │                           │
│  │  facelink-login                         │                           │
│  │                                         │                           │
│  │  สิทธิ์การเข้าถึงที่ร้องขอ:              │                           │
│  │  ☑ ข้อมูลโปรไฟล์ (ต้องการ)              │                           │
│  │                                         │                           │
│  │  สำหรับผู้ใช้ (ต้องการ):                 │                           │
│  │  ☐ facelink  [Add friend checkbox]      │  ← bot_prompt=aggressive  │
│  │                                         │                           │
│  │         [อนุญาต / Allow]                │                           │
│  └─────────────────────────────────────────┘                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼ User allows + checks "Add friend"
┌─────────────────────────────────────────────────────────────────────────┐
│  STEP 5: Add Friend Screen (if not already friend)                     │
│                                                                         │
│  ┌─────────────────────────────────────────┐                           │
│  │  LINE                                   │                           │
│  │  ─────────────────────────────────────  │                           │
│  │  ☑ facelink                             │                           │
│  │                                         │                           │
│  │  [เพิ่มเพื่อน / Add Friend]              │                           │
│  │                                         │                           │
│  │         [กลับ / Back]                   │                           │
│  └─────────────────────────────────────────┘                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼ Redirect back to our app
┌─────────────────────────────────────────────────────────────────────────┐
│  STEP 6-7: Back on our app                                             │
│                                                                         │
│  ☑ Select photos                                                       │
│  [ส่งรูปผ่าน Line] ← Click again                                        │
│                                                                         │
│  ✓ ส่งสำเร็จ / Sent successfully!                                       │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STEP 8: Photos arrive in LINE chat                                    │
│                                                                         │
│  ┌─────────────────────────────────────────┐                           │
│  │  facelink                               │                           │
│  │  ─────────────────────────────────────  │                           │
│  │                                         │                           │
│  │  [Photo carousel / Flex message]        │                           │
│  │  🖼️ 🖼️ 🖼️                               │                           │
│  │                                         │                           │
│  │  [Link to gallery]                      │                           │
│  └─────────────────────────────────────────┘                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

### 8b. Technical Flow (Sequence Diagram)

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌────────┐     ┌─────────┐     ┌─────────┐
│ Web App │     │LINE Auth│     │   API   │     │  D.O.  │     │   DB    │     │LINE Msg │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬───┘     └────┬────┘     └────┬────┘
     │               │               │               │               │               │
     │  1. Click "Send via LINE"     │               │               │               │
     │               │               │               │               │               │
     │  2. Check: logged in + friend?│               │               │               │
     │───────────────────────────────►               │               │               │
     │               │               │───────────────────────────────►               │
     │               │               │               │               │               │
     │  3a. If YES: skip to step 10  │               │               │               │
     │  3b. If NO: redirect to LINE Login            │               │               │
     │──────────────►│               │               │               │               │
     │               │               │               │               │               │
     │  4. LINE Login + bot_prompt=aggressive        │               │               │
     │  (consent screen with "Add friend" option)    │               │               │
     │◄─────────────►│               │               │               │               │
     │               │               │               │               │               │
     │  5. User approves + adds friend               │               │               │
     │◄──────────────│               │               │               │               │
     │               │               │               │               │               │
     │  6. Redirect back with auth code              │               │               │
     │◄──────────────│               │               │               │               │
     │               │               │               │               │               │
     │  7. Exchange code for token + get userId      │               │               │
     │───────────────────────────────►               │               │               │
     │               │               │               │               │               │
     │               │               │  8. Create/update participant │               │
     │               │               │  (line_user_id, line_linked=true)             │
     │               │               │───────────────────────────────►               │
     │               │               │               │               │               │
     │  9. Request: send photos to LINE              │               │               │
     │───────────────────────────────►               │               │               │
     │               │               │               │               │               │
     │               │               │  10. CHECK RATE LIMIT         │               │
     │               │               │  (per-user: 10/min, Durable Object)           │
     │               │               │──────────────►│               │               │
     │               │               │               │               │               │
     │               │               │  If exceeded → 429            │               │
     │               │               │◄──────────────┘               │               │
     │               │               │               │               │               │
     │               │               │  11. Verify line_linked=true  │               │
     │               │               │───────────────────────────────►               │
     │               │               │               │               │               │
     │               │               │  12. Build Flex Message       │               │
     │               │               │  (image carousel, max 12)     │               │
     │               │               │               │               │               │
     │               │               │  13. Push message via API     │               │
     │               │               │───────────────────────────────────────────────►
     │               │               │               │               │               │
     │               │               │  14. Log notification         │               │
     │               │               │  (photographer_id, photo_count, ip_address)   │
     │               │               │───────────────────────────────►               │
     │               │               │               │               │               │
     │  15. "ส่งสำเร็จ!" (Sent!)     │               │               │               │
     │◄──────────────────────────────│               │               │               │
     │               │               │               │               │               │
```

**Key validation order:**
1. **User authentication check** - must be logged in with LINE
2. **Friendship check** - must be friend of OA (`line_linked=true`)
3. **Rate limit check** (Durable Object, per-user) → `RATE_LIMITED` (429)
   - 10 requests/min per user
4. Verify `line_linked=true` → `LINE_NOT_LINKED` (422)

### 8c. Returning User Flow (Already signed in + friend)

```
┌─────────┐     ┌─────────┐     ┌────────┐     ┌─────────┐     ┌─────────┐
│ Web App │     │   API   │     │  D.O.  │     │   DB    │     │LINE Msg │
└────┬────┘     └────┬────┘     └────┬───┘     └────┬────┘     └────┬────┘
     │               │               │               │               │
     │  1. Click "Send via LINE"     │               │               │
     │───────────────►               │               │               │
     │               │               │               │               │
     │               │  2. CHECK RATE LIMIT          │               │
     │               │  (per-user: 10/min, Durable Object)           │
     │               │──────────────►│               │               │
     │               │               │               │               │
     │               │  If exceeded → 429            │               │
     │               │◄──────────────┘               │               │
     │               │               │               │               │
     │               │  3. Check participant         │               │
     │               │  (line_linked=true? ✓)        │               │
     │               │───────────────────────────────►               │
     │               │               │               │               │
     │               │  4. Build + send immediately  │               │
     │               │───────────────────────────────────────────────►
     │               │               │               │               │
     │               │  5. Log notification          │               │
     │               │───────────────────────────────►               │
     │               │               │               │               │
     │  6. "ส่งสำเร็จ!"               │               │               │
     │◄──────────────│               │               │               │
     │               │               │               │               │
```

**Key points:**
- LINE Login uses `bot_prompt=aggressive` to show separate "Add friend" screen after consent
- If user doesn't add friend, we store `line_linked=false` and prompt again next time
- Silent failure if we try to send to non-friend (API returns 200 but no delivery)
- Always check `line_linked=true` before attempting push message

**LINE Login URL parameters:**
```
https://access.line.me/oauth2/v2.1/authorize?
  response_type=code
  &client_id={CHANNEL_ID}
  &redirect_uri={CALLBACK_URL}
  &state={STATE}
  &scope=profile
  &bot_prompt=aggressive
```

---

## 9. Event Expiry Flow (Cron)

**(Previously flow 10 - LIFF flow removed)**

```
┌─────────┐     ┌─────────┐     ┌─────────┐     ┌─────────┐
│  Cron   │     │   DB    │     │   R2    │     │  Rekog  │
└────┬────┘     └────┬────┘     └────┬────┘     └────┬────┘
     │               │               │               │
     │  1. Run daily at 00:00 BKK    │               │
     │               │               │               │
     │  2. Query expired events      │               │
     │  WHERE start_datetime + 1 month < NOW()       │
     │  AND NOT already_expired      │               │
     │──────────────►│               │               │
     │               │               │               │
     │  3. For each expired event:   │               │
     │               │               │               │
     │    4. Delete Rekognition collection           │
     │───────────────────────────────────────────────►
     │               │               │               │
     │    5. List R2 objects for event               │
     │───────────────────────────────►               │
     │               │               │               │
     │    6. Delete R2 objects       │               │
     │───────────────────────────────►               │
     │               │               │               │
     │    7. Delete face records     │               │
     │──────────────►│               │               │
     │               │               │               │
     │    8. Delete search_results   │               │
     │──────────────►│               │               │
     │               │               │               │
     │    9. Delete search_sessions  │               │
     │──────────────►│               │               │
     │               │               │               │
     │    10. Mark event as expired  │               │
     │    (keep record, clear data)  │               │
     │──────────────►│               │               │
     │               │               │               │
     │  11. Log completion           │               │
     │               │               │               │
```

**Notes:**
- Run via Cloudflare Workers Cron Trigger
- Process in batches to avoid timeout
- Event record preserved (for history/accounting)

---

## 10. Credit Expiry Flow (Cron)

```
┌─────────┐     ┌─────────┐
│  Cron   │     │   DB    │
└────┬────┘     └────┬────┘
     │               │
     │  1. Run daily at 00:00 BKK
     │               │
     │  2. Find expiring credit batches
     │  WHERE expires_at <= NOW()
     │  AND type IN ('purchase', 'bonus', 'promo')
     │  AND NOT already_expired
     │──────────────►│
     │               │
     │  3. For each expiring batch:
     │               │
     │    4. Calculate remaining from this batch
     │    (using FIFO consumption tracking)
     │──────────────►│
     │               │
     │    5. If remaining > 0:
     │       Insert ledger entry
     │       (type='expired', amount=-remaining)
     │──────────────►│
     │               │
     │    6. Mark batch as expired
     │──────────────►│
     │               │
     │  7. Send low-balance alerts if needed
     │               │
```

**FIFO tracking:**
- Each credit batch has ID and `expires_at`
- When consuming, track which batch credits came from
- On expiry, only expire unused portion of that batch

---

## Flow Summary

| # | Flow | Trigger | Key Components |
|---|------|---------|----------------|
| 1 | Photographer Signup | User action | Clerk → API → DB |
| 2 | Credit Purchase | User action | Stripe → Webhook → DB |
| 3 | Create & Publish Event | User action | API → DB → Rekognition |
| 4 | Photo Upload (Web) | User action | API → DB → R2 → Rekognition |
| 5 | Photo Upload (FTP) | Camera upload | FTP → API → ... |
| 6 | Face Search (Web) | User action | API → Rekognition → DB |
| 7 | Photo Download | User action | API → CDN |
| 8 | LINE Notification | User action | Clerk LINE Login → API → LINE Messaging |
| 9 | Event Expiry | Cron (daily) | Cron → DB → R2 → Rekognition |
| 10 | Credit Expiry | Cron (daily) | Cron → DB |

**Note:** LIFF flow removed from MVP to reduce complexity. All participant auth goes through Clerk.
