import { pgTable, text, index, uuid, unique } from 'drizzle-orm/pg-core';
...
export const promoCodeUsage = pgTable(
  ...
  (table) => [
    ...
    unique('promo_code_usage_photographer_code_unique').on(table.photographerId, table.promoCode),
    // Index for fast lookup when checking if code was used
    index('promo_code_usage_photographer_code_idx').on(table.photographerId, table.promoCode),
