...
describe('LINE Webhook Signature Verification', () => {
  it('accepts valid signature', async () => {
    ...
    const isValid = await verifyLineSignature(body, signature, TEST_CHANNEL_SECRET);
  ...
  it('rejects invalid signature', async () => {
    ...
    const isValid = await verifyLineSignature(body, invalidSignature, TEST_CHANNEL_SECRET);
  ...
  it('rejects tampered body', async () => {
    ...
    const signature = await generateValidSignature(originalBody, TEST_CHANNEL_SECRET);
    ...
    const isValid = await verifyLineSignature(tamperedBody, signature, TEST_CHANNEL_SECRET);
  ...
  it('rejects wrong channel secret', async () => {
    ...
    const isValid = await verifyLineSignature(body, signature, 'wrong_channel_secret');
...
describe('LINE Webhook Edge Cases', () => {
  ...
  it('is case-sensitive for signature comparison', async () => {
    ...
    if (signature !== modifiedSignature) {
      const isValid = await verifyLineSignature(body, modifiedSignature, TEST_CHANNEL_SECRET);
      expect(isValid).toBe(false);
    }
  });
